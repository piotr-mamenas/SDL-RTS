...
{
private:
	std::unique_ptr<SDL_Texture> _loadTexture(std::unique_ptr<std::string> path);
	void _loadGameResources();
	void _releaseGameResources();
	void _releaseMap(std::map<int, std::unique_ptr<Sprite>> spriteMap);
	std::map<int, std::unique_ptr<Sprite>> _loadSprite(std::unique_ptr<std::string> fileName);
	std::unique_ptr<SDL_Renderer> _gameRenderer;
	std::map<int, std::unique_ptr<Sprite>> _sprites;
public:
	GameAssetManager(std::unique_ptr<SDL_Renderer> gameRenderer);
	~GameAssetManager();
