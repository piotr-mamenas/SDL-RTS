...
private:
	...
	std::vector<std::shared_ptr<Terrain>> _mapTerrain;
	std::vector<std::shared_ptr<Terrain>> _mapObjects;
	std::vector<std::shared_ptr<Unit>> _preplacedMapUnits;
	std::vector<std::shared_ptr<SDL_Rect>> _blockedArea;
	int _mapWidth;
	int _mapHeight;
...
public:
	...
	void getUnits(std::shared_ptr<SDL_Rect> containingBox);
	std::vector<std::shared_ptr<Terrain>> getTerrain();
